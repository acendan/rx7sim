...
const particleSystem = {
    ...
    update: (deltaTime, engineState) => {
        ...
        particleSystem.emitters.forEach(emitter => {
            if (engineState !== DriveState.STOP) {
                emitter.enabled = true
                emitter.settings.add_time = engineState === DriveState.ACCEL ? 0.01 : 0.02 // More particles during acceleration  
                emitter.settings.speed_from = engineState === DriveState.ACCEL ? 0.005 : 0.003
                emitter.settings.speed_to = engineState === DriveState.ACCEL ? 0.008 : 0.006
            } else {
        ...
        for (let i = particles_smoke_a.length - 1; i >= 0; i--) {
            const p = particles_smoke_a[i]
